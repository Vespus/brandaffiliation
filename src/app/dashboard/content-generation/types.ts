...
export const PartialMetaOutputSchema = z.object({
    meta: z.object({
        ...
        }).optional().describe("Twitter metadata"),
        robot: z.string().optional().describe("Robot meta tag content")
    }).optional().describe("Meta information"),
    descriptions: z.object({
        header: z.string().optional().describe("Banner Hero Header description in html with *one h1* and at least *two p* tags. paragraphs should be minimum 50-100 words long"),
        footer: z.string().optional().describe("Bottom of the page Hero Footer description")
    }).optional().describe("Content descriptions")
})
