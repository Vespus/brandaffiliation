...
import { db } from '@/db';
import { datasources, datasourceValues } from '@/db/schema';
...
export const addDatasource = actionClient
...
export const updateDatasource = actionClient
    ...
    .action(async ({ parsedInput }) => {
        try {
            ...
            await db.delete(datasourceValues)
                .where(eq(datasourceValues.datasourceId, parsedInput.id));
