...
export const NeighborScaleType = async ({
    brand,
    searchParams,
}: {
    ...
    const metrics = (a: BrandWithCharacteristicAndScales, b: BrandWithCharacteristicAndScales) =>
        Object.keys(DEFAULT_SCALE_WEIGHTS).map((scale) => ({
            label: t(`scale.${scale}`),
                            ...
                            <CardHeader className="pb-2">
                                <CardTitle className="flex items-center justify-between">
                                    ...
                                    <Badge className="ml-2 bg-green-600">
                                        {formatSimilarity(neighborBrand.combined_similarity)}
                                    </Badge>
                                </CardTitle>
                                    ...
                                    <ul className="flex flex-col gap-2">
                                        {neighborBrand.characteristic?.map((char) => (
                                            <div key={char.id} className="flex items-start gap-2 text-xs">
                                                <ArrowBigRightDash size={12} className="mt-0.5 flex-none" />
        ...
        <div className="space-y-1">
            <div className="flex flex-col justify-between">
                ...
                <div className="flex justify-between tabular-nums">
                    ...
                    >
                        {Math.round(percentage)}%{isExceeding && <ArrowUp size={12} />}
                    </span>
                </div>
            </div>
            <Progress
