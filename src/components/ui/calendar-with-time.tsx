...
export const CalendarWithTime = ({ value, onValueChange, ...props }: CalendarWithTimeProps) => {
                    ...
                    <div className="absolute inset-0 py-4 max-sm:border-t">
                        <Scroller className="h-full sm:border-s">
                            <div className="space-y-3">
                                <div className="flex h-5 shrink-0 items-center px-5"></div>
                                <div className="grid gap-1.5 px-5 max-sm:grid-cols-2">
                                    {timeSlots.map(({ time: timeSlot, available }) => (
                                        <Button
                                            key={timeSlot}
                                            variant={
                                                (selectedDate ? format(selectedDate, 'HH:mm') === timeSlot : false)
                                                    ? 'default'
                                                    : 'outline'
                                            }
                                            size="sm"
