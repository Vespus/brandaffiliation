...
export const getReviewTasks = cache(async () => {
    const reviewContents = await db
        ...
        .leftJoin(brands, and(or(eq(contents.entityType, "combination"), eq(contents.entityType, "brand")), or(eq(contents.entityId, brands.integrationId), eq(brands.integrationId, combinations.brandId))))
