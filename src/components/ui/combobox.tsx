...
>(({ className, children, ...props }, ref) => (
    <ComboboxPrimitive.BadgeItem
        ...
        ref={ref}
        className={cn('bg-secondary inline-flex items-center justify-between gap-1 rounded-sm px-2 py-0.5', className)}
        {...props}
    >
        <span className="text-secondary-foreground truncate text-[13px]">{children}</span>
...
>(({ className, children, outset, ...props }, ref) => (
    ...
    >
        ...
        <ComboboxPrimitive.ItemText asChild>{children}</ComboboxPrimitive.ItemText>
    </ComboboxPrimitive.Item>
))
