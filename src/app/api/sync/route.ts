...
const processBrands = async (brands: QSPayBrand[]) => {
    ...
    for (const remoteBrand of brands) {
        const isExists = localBrands.find(
            (x) => x.slug?.toLowerCase() === remoteBrand.slug.toLowerCase().replace(/\//g, '')
        )
        if (!isExists) {
    ...
    await db.transaction(async (tx) => {
        if (insertDiff.length > 0) {
            await tx.insert(localBrandsTable).values(
                insertDiff.map((brand) => ({
...
const processCategories = async (categories: QSPayCategory[]) => {
    ...
    for (const remoteCategory of categoryFlat(categories)) {
        const isExists = localCategories.find(
            (x) => x.slug?.toLowerCase() === remoteCategory.slug.toLowerCase().replace(/\//g, '')
        )
        if (!isExists) {
    ...
    await db.transaction(async (tx) => {
        if (insertDiff.length > 0) {
            await tx.insert(localCategoriesTable).values(
                insertDiff.map((category) => ({
                    name: category.name,
...
const processCombinations = async (combinations: QSPayCombin[]) => {
    ...
    return await db.transaction(async (tx) => {
        await tx.insert(localCombinationsTable).values(
