...
export const QspayApplyDialog = () => {
    ...
    const fieldMappings = [
        ...
        },
        {
            title: 'Opengraph',
            description:
                'Meta data for sharing the page on social media platforms like Facebook and Twitter or Whatsapp',
            children: [
        ...
        },
        {
            ...
            description:
                'Content for page. These will appear next to banner or footer of the selected entity(category, brand, etc.)',
            children: [
    ...
    return (
        <Dialog open={Boolean(selectedStream)} onOpenChange={() => setParams({ selectedStream: null })}>
            <DialogContent
                className="max-h-[90vh] w-full overflow-hidden lg:max-w-7xl"
                                    ...
                                    <Card key={title}>
                                        <CardHeader>
                                            ...
                                            <CardContent className="flex flex-col gap-4 px-0 py-4">
                                                <div className="bg-background sticky top-0 flex gap-4 py-4">
                                                ...
                                                {children.map(({ path, label, type: Component }) => (
                                                    <div
                                                        key={path}
                                                        className="flex min-w-0 gap-4 border-b pb-4 last:border-b-0"
                                                    >
                                                        ...
                                                        <div
                                                            className="text-muted-foreground prose prose-sm dark:prose-invert flex-1 text-xs [&>h1]:text-xs"
                                                            dangerouslySetInnerHTML={{
                                                                __html: get(entityData, path) || 'Not available',
                                                            }}
                                                        />
                                                    </div>
                                                ))}
