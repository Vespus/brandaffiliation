...
import { datasourceValues, datasources, systemPrompts } from '@/db/schema'
...
export const CompletionStream = async (parsedInput: z.infer<typeof ContentGenerateSchema>) => {
    ...
    const [models, prompt, { result: brand }, { result: category }, { result: store }] = await Promise.all([
    ...
    if (queryableDataSources) {
        for await (const source of queryableDataSources) {
            ...
            dataSourcePrompt = appendMarkdown(
                dataSourcePrompt,
                `<Datasource>${source.datasourcePrompt}: ${ds.data.join(', ')}</Datasource>`
            )
        }
    }
...
export const SaveToQSPay = actionClient
    ...
    .action(async ({ parsedInput: { brandId, brandName, categoryName, categoryId, output } }) => {
        if (brandId && categoryId) {
            ...
            const foundCombinationPage = allCombinationPages.find(
                (cp) => cp.category?.id === categoryId && cp.brand?.id === brandId
            )

            if (!foundCombinationPage) {
                throw new Error(
                    "Combination page not found. BrandAffiliation can't create a new one, please create one manually then try again."
                )
            }
        ...
        if (brandId && !categoryId) {
            ...
            if (!foundBrandPage) {
                throw new Error(
                    "Brand page not found. BrandAffiliation can't create a new one, please create one manually then try again."
                )
            }
        ...
        if (!brandId && categoryId) {
            ...
            if (!foundCategoryPage) {
                throw new Error(
                    "Category page not found. BrandAffiliation can't create a new one, please create one manually then try again."
                )
            }
