...
export function getUsersTableColumns(): ColumnDef<User>[] {
    return [
        ...
        },
        {
            id: 'image',
            accessorKey: 'image',
            header: ({ column }) => <DataTableColumnHeader column={column} title="Profile Picture" />,
            cell: ({ row }) => {
                ...
                return (
                    <Avatar className="size-8 rounded-lg">
                        {imageUrl && (
                            <AvatarImage src={imageUrl} alt={row.original.name} className="aspect-auto object-cover" />
                        )}
        ...
        },
        {
            ...
            cell: ({
                ...
                return <div>{meta?.formatter?.dateTime(new Date(row.getValue('createdAt')))}</div>
            },
            enableSorting: true,
            enableHiding: false,
            enableColumnFilter: true,
