...
export const genericRoute = createTRPCRouter({
    getBrandsWithCharacteristics: publicProcedure.query(async () => {
    ...
    getAIModels: publicProcedure.query(async () => {
    ...
    getPrompt: publicProcedure
        ...
        .query(async ({ input }) => {
            const [brand] = await db.select().from(brandWithScales).where(eq(brandTable.id, input.brand))
    ...
    getUserPrompts: publicProcedure.query(async () => {
    ...
    getTranslationById: publicProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
        return db.query.translations.findFirst({ where: eq(translations.id, input.id) })
    }),
    getBrand: publicProcedure.input(z.number()).query(async ({ input }) => {
    ...
    getDatasourceById: publicProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    ...
    getDatasourceValues: publicProcedure
        ...
        .query(async ({ input }) => {
            return db.query.datasourceValues.findMany({
                where: eq(datasourceValues.datasourceId, input.datasourceId),
            })
        }),
    getSystemPrompts: publicProcedure.query(async () => {
