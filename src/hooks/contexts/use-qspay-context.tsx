...
export function QSPayContextProvider({children, user, storeId}: QSPayContextProps) {
    const currentStore = user && storeId ? user.companies[0]?.merchants[0]?.stores.find(store => store.storeId === storeId) : null;
    ...
    return (
        <QSPayContext.Provider value={{user, currentStore}}>
            {children}
        </QSPayContext.Provider>
    );
}
